const { xorDecryption } = require("../lib");

const RESULTS = [
  {
    data: [0, 0, 0, 31, 0, 1, 25, 89, 73, 24, 69, 15, 8, 11, 89, 75, 2, 22, 2, 29],
    key: "my key",
    result: "my text is very good",
    iterations: 1,
    ...
    result: JSON.stringify({ i: { can: { encrypt: { serialized: "data" }}}}),
    iterations: 1,
    outputAsString: false,
  }, {
    data: "{\"i\":{\"can\":{\"encrypt\":{\"serialized\":\"data\"}}}}",
    key: "I'm another very secure key",
    ...
    data: [43, 15, 38, 24, 4, 9],
    key: [1, 2, 3],
    result: [42, 13, 37, 25, 6, 10],
    iterations: 1,
    outputAsString: false,
  },
];

describe("xorDecryption", () => {
  RESULTS.forEach(entry => {
    test(`xoring '${entry.data}' with key '${entry.key}' should yield '${entry.result}' (${entry.iterations} iterations)`, () => {
      ...
      for (let i = 0; i < entry.iterations; i++) {
        res = xorDecryption(res, entry.key, entry.outputAsString);
      }
