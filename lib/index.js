...
function phone(object) {
  ...
  for (var i = 0; i < object.value.length; i++) {
    ...
    if (conditional) {
      returnVal['userPhones-' + i] = (0, _helpers.getResponse)(object, object.type);
    }
  }
...
function loginEmail(object, prop) {
  ...
  if (conditional) {
    returnVal = (0, _helpers.getResponse)(object, object.type);
  }
...
function email(object) {
  ...
  for (var i = 0; i < object.value.length; i++) {
    ...
    if (conditional) {
      returnVal['userEmails-' + i] = (0, _helpers.getResponse)(object, object.type);
    }
  }
...
function string(object, prop) {
  ...
  if (conditional) {
    returnVal = (0, _helpers.getResponse)(object, prop);
  }
